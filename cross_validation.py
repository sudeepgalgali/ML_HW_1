...
def split_cv(length, num_folds):
    ...
    addition_factor=int(length/num_folds)
    
    for i in range(num_folds):
        splits[i][1].extend(indices[i*addition_factor:(i+1)*addition_factor])
        splits[i][0].extend(indices[0:i*addition_factor]+indices[(i+1)*addition_factor:length])
